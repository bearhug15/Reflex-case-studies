...
proof(induction s3)
 ...
 assume 1: "substate s1 s2 \<and> substate s2 emptyState"
  ...
  next
    case (setVar s3 v u)
      assume 1: "substate s1 s2 \<and> substate s2 (setVar s3 v u)"
      show ?case
      proof(cases)
        assume "s2 = setVar s3 v u"
        with 1 show ?case by auto
      next
        assume "s2 \<noteq> setVar s3 v u"
        with 1 setVar.IH show ?case by auto
      qed
  next
...
next
  case (setVar s2 v u)
    assume 1: "substate s1 (setVar s2 v u) \<and> substate (setVar s2 v u) s1"
  show ?case
  proof(cases)
    assume "s1 = setVar s2 v u"
    thus ?case by assumption
  next
    assume 2: "s1 \<noteq> setVar s2 v u"
    from 1 obtain 3: "substate s1 (setVar s2 v u)" 
and 4: "substate (setVar s2 v u) s1" ..
    from 2 3 have "substate s1 s2" by simp
    with 4 substate_trans 
    have 5: "substate (setVar s2 v u) s2" by blast
    have "substate s2 (setVar s2 v u)" 
      using substate_refl by auto
    with 5 setVar.IH have "setVar s2 v u = s2" by auto
    with 3 4 setVar.IH show ?case by auto
  qed
next
...
proof(cases)
  ...
  next
    assume 1: "\<not> toEnvP s"
assume "toEnvNum  emptyState (setVar s v u) > 1"
 ...
 \<Longrightarrow> toEnvNum s1 s3 = toEnvNum s1 s2 + toEnvNum s2 s3"
  ...
  next
    ...
    proof(cases)
      ...
      thus ?case by auto
    next
      assume 1: "s2 \<noteq> setVar s3 v u"
      assume 2:  "substate s1 s2 \<and> substate s2
 (setVar s3 v u)"
      with 1 have "substate s2 s3" by simp
      show ?case
      proof(cases)
        assume "s1 = setVar s3 v u"
        with 2 substate_asym[of "(setVar s3 v u)" s2]
        have "setVar s3 v u = s2" by auto
        with 1 show ?case by auto
      next
        assume "s1 \<noteq> (setVar s3 v u)"
        with 1 2 setVar.IH show?case by auto
      qed
    qed
  next
    case (setPstate s3 p q)
...
| "timeContinuous s (setVar s1 v u) p = 
  (if s = setVar s1 v u then True else timeContinuous s s1 p)" 
