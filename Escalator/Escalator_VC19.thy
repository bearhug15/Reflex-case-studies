...
next
  ...
  proof -
    ...
    moreover have "R3A st_final st0" 
    proof (simp add: R3A_def; rule impI)
      assume prems:"getVarBool st0 ''out_0'' = True \<and>
            (getVarBool st_final ''inp_0'' \<or>
             getVarBool st_final ''inp_1'') \<and>
            \<not> getVarBool st0 ''inp_3'' \<and>
            \<not> getVarBool st0 ''inp_4'' \<and>
            \<not> getVarBool st_final ''inp_3'' \<and>
            \<not> getVarBool st_final ''inp_4''"
      have "getPstate st0 ''Ctrl'' = ''stuckState''" using assms by auto
      moreover have "getPstate (predEnv st0) ''Ctrl'' = ''stuckState''" using Pstate_continuation assms predEnv_toEnvNum
        by auto
      ultimately have "getVarBool st0 ''out_0'' = True" using extra0 extraInv eStuckStateOut_def substate_refl by
    qed
